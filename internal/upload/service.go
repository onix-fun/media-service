...
func (s *service) GetDownloadURL(ctx context.Context, blobID uuid.UUID, createdBy string) (string, error) {
	...
	serviceOwnsBlob := !strings.Contains(createdBy, ":") && strings.HasPrefix(blob.CreatedByService, createdBy+":")
	if createdBy != blob.CreatedByService && !serviceOwnsBlob {
		allowed, err := s.metadata.HasBlobAccess(ctx, blobID, createdBy)
		if err != nil {
			return "", fmt.Errorf("failed to verify blob ownership: %w", err)
		}
		if !allowed {
