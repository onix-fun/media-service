...
func (s *blobStorage) GeneratePresignedPartURL(ctx context.Context, key string, uploadID string, partNumber int, expiry time.Duration) (string, error) {
	...
	// Multipart PUTs are performed by the browser, so a URL signed against the
	// private in-cluster endpoint (for example minio:9000) is unusable outside
	// Docker. Sign it with the configured public endpoint just like downloads.
